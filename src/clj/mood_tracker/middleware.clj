(ns mood-tracker.middleware
  (:require [camel-snake-kebab.core :as csk]
            [clojure.walk :refer [postwalk]]))

(defn case-to-kebab [body]
  (if-not (empty? body)
    (->> body
...
(defn body-case-middleware [handler]
  (fn [{{:keys [body]} :parameters :as request}]
    (let [body (case-to-kebab body)
          request (assoc-in request [:parameters :body] body)
          response (handler request)]
      response)))

(comment
  (let [body {:first_name "afd"
              :last_name "asdfasdf"
              :shit_name "asdfasd"
              ::shit_name "adf"}
        r (case-to-kebab body)]
    r)
  (let [h (fn [& all] all)
        r {:parameters {:body {:first_name "afd"
                               :last_name "asdfasdf"
                               :shit_name "asdfasd"}}}
        res ((body-case-middleware h) r)]
    res)
  )
